...
vector<double> conditions = {0,   10,  50,  100, 200, 300, 400,
...
bool DoubleEquals(double x, double y) { return fabs(x - y) < .001; }
...
CreateSequence(const pcl::PointCloud<pcl::PointXYZ> &cloud,
...
SubsamplePlane(pcl::PointCloud<pcl::PointXYZ> cloud, int axis, int count) {
...
void RunConditionExperiment(string file, pcl::PointCloud<pcl::PointXYZ> cloud,
                            vector<vector<double>> deltas_1,
                            vector<vector<double>> deltas_2) {
  cout << "creating sequence" << endl;
  vector<pcl::PointCloud<pcl::PointXYZ>> clouds =
      CreateSequence(cloud, deltas_1);
  double time = 0;
  ...
  for (size_t i = 0; i < 1; ++i) {
    ...
    for (size_t j = 0; j < clouds.size() / 2; j = j + 2) {
      ...
      cout << "run icp" << endl;
      ICP(10, .5, publishers, "", "", cloud2, cloud1, normals_2, normals_1,
          empty_coords, empty_coords, transform, variance, NULL);
      ...
      // Writes both deltas to the same file
      WritePoseFile(transform, j, j, pose_file);
      double transform_array[6];
      std::copy(deltas_2[count].begin(), deltas_2[count].end(),
                transform_array);
...
int main(int argc, char **argv) {
  ...
  partial_calibrate::ReadDeltasFromFile(file, &deltas_1, &deltas_2,
                                        &uncertaintyE_1, &uncertaintyE_2);
