...
// Transforms a given eigen point by the given transform (array input)
template <class T> Eigen::Matrix<T,2,1> TransformDifference(
    const T transform_base[6],
    const T transform[6]) {
    ...
    Eigen::Transform<T, 3, Eigen::Affine>(Eigen::AngleAxis<T>(
        angle, axis));
    ...
    Eigen::Matrix<T,3,1> axis_base(transform_base[0], transform_base[1], transform_base[2]);
    ...
    Eigen::Translation<T, 3> translation_base =
        Eigen::Translation<T, 3>(transform_base[3], transform_base[4], transform_base[5]);
    ...
    // Compute Translation
    Eigen::Matrix<T, 3,1> combined_translation(
...
//       double res_magnitude = sqrt(pow(res_dist,2) + pow(res_dist2,2));
      ...
      if (abs(dist) < nn_dist && abs(res_dist) < nn_dist && abs(res_dist2) < nn_dist) {
...
// //     // Building a histogram to adjust nn dist (testing, should be moved outside of nearest neighbor calculation)
//     std::stringstream out;
// //     FILE* fp;
// //     if(first_nn) {
// //       out << count;
// //       string name = "raw_residuals_" + out.str() + ".txt";
// //       count++;
// //       fp = fopen( name.c_str(), "w" ); // Open file for writing
// //     }
//     sort(residual_magnitudes.begin(), residual_magnitudes.end());
//     double current_residual = residual_magnitudes[0];
//     vector<int> frequency;
//     frequency.push_back(1);
//     vector<double> bin_min;
//     bin_min.push_back(current_residual);
//     for(size_t i = 1; i < residual_magnitudes.size(); i++) {
//       double residual = residual_magnitudes[i];
// //       if (first_nn) {
// //         fprintf(fp, "%f \n", residual);
// //       }
//       if(HistogramEquals(current_residual, residual)){
//         frequency.back() += 1;
//       } else {
//         current_residual = residual;
//         frequency.push_back(1);
//         bin_min.push_back(residual);
//       }
//     }
//     //fprintf(fp, "Bin \t Frequency \t Cumulative \n");
//     int sum = 0;
//     int total = residual_magnitudes.size();
//     double threshold = 9999;
//     for(size_t i = 2; i < frequency.size(); i++) {
//        double min = bin_min[i];
//        int freq = frequency[i];
//        sum += freq;
//        double cumulative = (double)sum / ((double)total - (double)frequency[0] - (double)frequency[1]);
//        if(cumulative > .98 && threshold == 9999){
//           threshold = min;
//        }
//        //fprintf(fp, " %f \t %d \t %f \n", min, freq, cumulative);
//
...
void TransformPointCloudQuat(
  ...
  for(size_t i = 0; i < cloud.size(); ++i) {
    ...
    Eigen::Matrix<double,3,1> transformed_point_matrix;
    point_matrix << point.x, point.y, point.z;
    transformed_point_matrix = TransformPointQuaternion(point_matrix, transform);
    point.x = transformed_point_matrix[0];
    point.y = transformed_point_matrix[1];
    point.z = transformed_point_matrix[2];
    cloud[i] = point;
...
struct PointToPointErrorNumeric {
  PointToPointErrorNumeric(const Vector3d& point_0,
...
struct PointToPointError {
  PointToPointError(const Vector3d& point_0,
                    const Vector3d& point_1,
                    const Vector3d& normal_0,
                    const Vector3d& normal_1) :
      point_0(point_0),
...
struct PlaneToPlaneError {
    ...
    bool operator()(const T* const camera,
        T* residuals) const {
            ...
            // The error is the difference between the predicted and observed position.
            residuals[0] = ceres::acos(point_1.dot(point_2_transformed) / ceres::sqrt(point_1.norm() * point_2_transformed.norm()));
            residuals[1] =
            (centroid_2_transformed - centroid_1.cast<T>()).dot(point_1);
            residuals[2] =
            (centroid_1.cast<T>() - centroid_2_transformed).dot(point_2_transformed);
            return true;
          }
    ...
    static ceres::CostFunction* Create(const Eigen::Vector4d plane_1,
        const Eigen::Vector4d plane_2, const Eigen::Vector3d centroid_1, const Eigen::Vector3d centroid_2) {
...
struct TransformRegularizationError {
    ...
    bool operator()(const T* const camera,
        T* residuals) const {
            const Eigen::Matrix<T, 3, 1> rotation = Eigen::Matrix<T, 3, 1>(
                camera[0],
                camera[1],
                camera[2]);
            const Eigen::Matrix<T, 3, 1> translation = Eigen::Matrix<T, 3, 1>(
                camera[3],
                camera[4],
                camera[5]);
...
void PlaneCorrections(
    ...
    for   ( int iteration = 0, repeat_iteration = 0;
        ...
        // Build the problem
        ceres::CostFunction* cost_function = PlaneToPlaneError::Create(
           normal_equations[0], k2_normal_equations[1], k1_centroids[0], k2_centroids[1]);
        problem.AddResidualBlock(cost_function,
                                 new ceres::HuberLoss(0.5), // squared loss
                                 transform);
        ceres::CostFunction* cost_function_2 = PlaneToPlaneError::Create(
            normal_equations[1], k2_normal_equations[0], k1_centroids[1], k2_centroids[0]);
        problem.AddResidualBlock(cost_function_2,
                                 new ceres::HuberLoss(0.5), // squared loss
                                 transform);
        ceres::CostFunction* cost_function_3 = TransformRegularizationError::Create(base_transform);
        if(not_first) {
  ...
  for (size_t i = 0; i < start_points.size(); i++) {
    ...
    if (distance < nn_dist && distance > 0) {
      ...
      const Vector3d normal_0(normal_k.normal_x,
                              normal_k.normal_y,
                              normal_k.normal_z);
      const Vector3d normal_1(normal_k1.normal_x,
                              normal_k1.normal_y,
                              normal_k1.normal_z);
      if (point_0.allFinite() &&
          point_1.allFinite() &&
          normal_0.allFinite() &&
          normal_1.allFinite()) {
        ceres::CostFunction* cost_function =
            PointToPointErrorNumeric::Create(point_0,
                                             point_1,
                                             normal_0,
                                             normal_1);
        problem->AddResidualBlock(cost_function,
...
void VisualizeNN(
  ...
  double norm = sqrt(pow(combined_result.x,2) + pow(combined_result.y,2) + pow(combined_result.z,2));
...
void VisualizeReferenceFrame(
  ...
  x_arrow.header.frame_id = y_arrow.header.frame_id = z_arrow.header.frame_id = "point_cloud";
  x_arrow.action = y_arrow.action = z_arrow.action = visualization_msgs::Marker::ADD;
  x_arrow.type = y_arrow.type = z_arrow.type = visualization_msgs::Marker::ARROW;
  x_arrow.id = id;
  y_arrow.id = id+1;
  z_arrow.id = id+2;
...
void VisualizeCovariance(
  ...
  out << num;
  string filename = covarianceFolder + "/" + bagfile + "_"
      + out.str() + ".covariance";
...
void ConstructICP_problem(
  ...
  //cout << "Finding Neighbors" << endl;
  KdTreeNN(nn_dist,cloud_1, transformed_cloud,
           normal_1,
           normal_2,
           image_coords_1,
           image_coords_2,
           nearest_neigbors,
           start_points);
  //VisualizeNN(cloud_1, cloud_2, nearest_neigbors, start_points, publishers[4]);
    ...
    if (distance < nn_dist) {
      ...
      if (point_0.allFinite() &&
        ...
        if (kUseNumericOverAutoDiff) {
          cost_function = PointToPointErrorNumeric::Create(
              point_0, point_1, normal_0, normal_1);
        } else {
          cost_function = PointToPointError::Create(
              point_0, point_1, normal_0, normal_1);
        }
        problem->AddResidualBlock(cost_function,
                                NULL, // squared loss
...
void ConstructICPKnown(
  ...
  for (size_t i = 0; i < cloud_2.size(); i++) {
    ...
    pcl::Normal normal_k1 = normal_2[i];
    int num;
    bool corresponds = GetKnownCor(cloud_1, normal_1, index_k2[i], index_k1,  &normal_k, &base_point, num);
...
//     fprintf(stdout, "kdtree run\n");
    //VisualizeNN(cloud_1, transformed_cloud, nearest_neigbors, start_points, publishers[4]);
  }
  fprintf(stdout, "RMSE: %f\n", rmse);
  if (final_rmse) *final_rmse = rmse;
}
...
rosbag::View::iterator GetCloudsSlamBag(rosbag::View::iterator it,
...
// Reads clouds from a given iterator, saves to buffer if they are over
rosbag::View::iterator GetCloudsOne(rosbag::View::iterator it,
                                 std::deque<pcl::PointCloud<pcl::PointXYZ> >* buffer1,
                                 std::deque<double>* timestamps_1) {
...
rosbag::View::iterator TimeAlignedClouds(rosbag::View::iterator it,
   rosbag::View::iterator end,
   std::deque<pcl::PointCloud<pcl::PointXYZ> >* buffer1,
   std::deque<pcl::PointCloud<pcl::PointXYZ> >* buffer2,
   std::deque<double>* timestamps_1,
...
rosbag::View::iterator TimeAlignedCloudsSlamBag(rosbag::View::iterator it,
                                                rosbag::View::iterator end,
...
bool CheckResidualDist(const pcl::PointCloud<pcl::PointXYZ>& k_cloud,
  ...
  double mean2 = abs(ResidualDist(k_cloud, keyframe, k_normal, key_normal, pose));
  if (mean2 >= threshold) {
    return true;
  }
  else {
...
void WriteToObj(const string folder, const string bagfile,
  ...
  for(size_t i = 0; i < cloud.size(); i++) {
    pcl::PointXYZ point = cloud[i];
    file << "v " << point.x << " " << point.y << " " << point.z << " 0 255 0" << endl;
  }
  file.close();
}
...
void WriteToBag(const string topic,
                rosbag::Bag* bag,
                const pcl::PointCloud<pcl::PointXYZ> cloud) {
  sensor_msgs::PointCloud2 temp_cloud;
  pcl::PCLPointCloud2 pcl_cloud;
