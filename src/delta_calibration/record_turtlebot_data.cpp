...
double global_min;
typedef actionlib::SimpleActionClient<frontier_exploration::ExploreTaskAction>
    Client;
enum Mode { monitor, record, record_rot, record_trans, find_scene, calibrate };

Mode mode = find_scene;
...
template <typename T> void PopFront(std::vector<T> *vec) {
  assert(!vec.empty());
  vec.erase(vec.begin());
}
...
bool DoubleEquals(double x, double y) { return fabs(x - y) < .3; }
...
double *DeltaFromOdom(double *current, double *previous) {
  ...
  Eigen::Quaternion<double> previous_quat(previous[3], previous[0], previous[1],
                                          previous[2]);
  ...
  Eigen::Quaternion<double> current_quat(current[3], current[0], current[1],
                                         current[2]);

  // Calculate delta rotation
  Eigen::Quaternion<double> rotation = previous_quat.inverse() * current_quat;

  Eigen::Quaternion<double> extrinsic_quat(extrinsics[3], extrinsics[0],
                                           extrinsics[1], extrinsics[2]);

  Eigen::Translation<double, 3> translation = Eigen::Translation<double, 3>(
      -cur_translation[0] + previous_translation.x(),
      -cur_translation[1] + previous_translation.y(),
      -cur_translation[2] + previous_translation.z());
  ...
  // Compute Translation
  Eigen::Translation<double, 3> combined_translation(transform.translation());
  Eigen::Vector3d combined_trans = {combined_translation.x(),
...
void CheckCalibration() {
  ...
  for (size_t i = 0; i < sensor_deltas.size(); i++) {
    ...
    Eigen::Vector3d odom_trans = {current_odom[4], current_odom[5],
                                  current_odom[6]};
    Eigen::Vector4d odom_rot = {current_odom[0], current_odom[1],
                                current_odom[2], current_odom[3]};
    // Transform the sensor delta
    Eigen::Vector3d sensor_trans = {current_sensor[4], current_sensor[5],
                                    current_sensor[6]};
    Eigen::Vector4d sensor_rot = {current_sensor[0], current_sensor[1],
                                  current_sensor[2], current_sensor[3]};
...
double *TransformTransform(double *base_transform, double *transform) {
  ...
  Eigen::Matrix<double, 3, 1> axis2(base_transform[0], base_transform[1],
                                    base_transform[2]);
  ...
  Eigen::Transform<double, 3, Eigen::Affine> rotation2 =
      Eigen::Transform<double, 3, Eigen::Affine>(
          Eigen::AngleAxis<double>(angle2, axis2));
  ...
  Vector3d trans_vec = {base_transform[3], base_transform[4],
                        base_transform[5]};
...
void DeltaErr() {
  ...
  if (DoubleEquals2(cloud_time, odom_time)) {
    ...
    if (DoubleEquals2(cloud_time_last, odom_time_last) &&
        (!DoubleEquals2(cloud_time_last, 0)) &&
      ...
      ICP(10, .1, publishers, "", "", last_cloud, global_cloud, normals_1,
          normals_2, empty_coords, empty_coords, calculated_delta, NULL);
      ...
      delta_calc::ExtractUncertainty(plane_normals, &uncertainty_t,
                                     &uncertainty_r);
      cout << "Uncertainty" << endl;
      cout << uncertainty_t << endl;
      delta_calc::StripUncertainty(uncertainty_t, uncertainty_r,
                                   calculated_delta);
      delta_calc::StripUncertainty(uncertainty_t, uncertainty_r, combined);

      Eigen::Matrix<double, 4, 1> error =
          TransformDifference(combined, calculated_delta);
      Eigen::Vector3d transformed_odom_rot = {combined[0], combined[1],
                                              combined[2]};
      Eigen::Vector3d transformed_odom_trans = {combined[3], combined[4],
                                                combined[5]};
      Eigen::Vector3d calculated_rot = {
          calculated_delta[0], calculated_delta[1], calculated_delta[2]};
      Eigen::Vector3d calculated_trans = {
          calculated_delta[3], calculated_delta[4], calculated_delta[5]};
    ...
    std::copy(std::begin(current_pose), std::end(current_pose),
              std::begin(last_pose));
  }
}
...
void CheckGroundPlane(const pcl::PointCloud<pcl::PointXYZ> &pcl_cloud) {
  ...
  Eigen::Vector3d normal_axis = {ground_normal[0], ground_normal[1],
                                 ground_normal[2]};
...
void SetAll() { x_orth = y_orth = z_orth = x_par = y_par = z_par = true; }
...
void CheckNormals(pcl::PointCloud<pcl::Normal> normals) {
  ...
  for (size_t i = 0; i < normals.size(); i++) {
    Eigen::Vector3d normal = {normals[i].normal_x, normals[i].normal_y,
                              normals[i].normal_z};
    normal.normalize();
...
bool AllInfo() { return x_orth && y_orth && z_orth && x_par && y_par && z_par; }
...
void CheckSceneInformation(const pcl::PointCloud<pcl::PointXYZ> &pcl_cloud) {
  ...
  if (condition < 15) {
    ...
    cancel_pub.publish(message);
    mode = record_trans;
    rot_bag.open("recorded_data_full_rot.bag", rosbag::bagmode::Write);
    trans_bag.open("recorded_data_full_trans.bag", rosbag::bagmode::Write);
    pthread_t drive_thread;
    num_passes = 20;
    ...
    string count_string = std::to_string(part_count);
    const string rot_name = "recorded_data_part_" + count_string + "_rot.bag";
    const string trans_name = "recorded_data_part_" + count_string + "_trans.bag";
    part_count++;
    rot_bag.open(rot_name, rosbag::bagmode::Write);
    trans_bag.open(trans_name, rosbag::bagmode::Write);
...
int main(int argc, char **argv) {
  ...
  rot_bag.close();
  trans_bag.close();
  ground_error_pub = n.advertise<std_msgs::Float32MultiArray>(
      "/calibration/ground_plane_error", 1000);
  calibration_error_pub = n.advertise<std_msgs::Float32MultiArray>(
      "/calibration/calibration_error", 1000);
  ...
  recalibrate_pub = n.advertise<geometry_msgs::PoseStamped>(
      "/calibration/extrinsic_update", 1);
  velocity_pub =
  ...
  ros::Subscriber command_sub =
      n.subscribe("/calibration/commands", 1, CommandCb);
  ros::Subscriber pertubation_sub =
      n.subscribe("/calibration/perturb", 1, PerturbCb);
