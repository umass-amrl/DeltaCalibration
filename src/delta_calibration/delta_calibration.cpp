...
void StripUncertainty(const Vector3d &ut,
                      const Vector3d &ur,
                      double *transform) {
  Eigen::Transform<double, 3, Eigen::Affine> R =
...
void CalculateDelta(const int k, const vector<ros::Publisher> &publishers,
  ...
  ICP(k, .05, publishers, covarianceFolder, bagFile, cloud_1, cloud_2, normal_1,
      normal_2, empty_coords, empty_coords, calculated_delta, NULL);
  Eigen::Matrix<double, 3, 1> axis(calculated_delta[0], calculated_delta[1],
                                   calculated_delta[2]);
  Eigen::Matrix<double, 3, 1> trans(calculated_delta[3], calculated_delta[4],
                                    calculated_delta[5]);
  ...
  *final_rmse = angle_degree;
  ...
  ICP(k, .05, publishers, covarianceFolder, bagFile, cloud_3, cloud_2, normal_3,
      normal_2, empty_coords, empty_coords, transform, NULL);

  double zero_pose[6];
  std::copy(pose0.begin(), pose0.end(), zero_pose);
}

void ExtractUncertainty(const vector<Eigen::Vector4d> &normal_equations,
  ...
  for (size_t i = 0; i < normal_equations.size(); i++) {
    ...
    Eigen::Vector3d normal = {normal_equation[0], normal_equation[1],
                              normal_equation[2]};
    if (normal_count > 0) {
      // If new normal.
      if (!DoubleEquals(abs(rot_u.dot(normal)), 1) &&
        ...
        // If we have two non-parallel normals, no rotation uncertainty.
        if (normal_count == 1) {
          rot_u[0] = 0;
          rot_u[1] = 0;
          rot_u[2] = 0;
      ...
      if (normal_count >= 2 && !DoubleEquals(abs(partial_u.dot(normal)), 0)) {
        trans_u = {0, 0, 0};
      }
...
rosbag::View::iterator InitializeVariables(string bag_name, int degree,
                                           rosbag::View::iterator bag_it,
                                           rosbag::View::iterator end,
  ...
  input->bag_name = bag_name + "_" + out.str();
  ...
  bag_it = TimeAlignedClouds(bag_it, end, &input->k1_buffer, &input->k2_buffer,
                             &(input->k1_timestamps), &input->k2_timestamps,
                             &input->k1_keyframe, &input->k2_keyframe,
                             &input->k1_timestamp, &input->k2_timestamp);
  // Key normals
...
rosbag::View::iterator InitializeVariablesSingle(string bag_name, int degree,
                                                 rosbag::View::iterator bag_it,
                                                 rosbag::View::iterator end,
                                                 DeltaCalVariables *input) {
  ...
  // and cloud k - 1, set the combined transform to the zero transform
  bag_it =
      OneSensorClouds(bag_it, end, &input->k1_buffer, &(input->k1_timestamps),
...
rosbag::View::iterator InitializeVariablesBrass(string bag_name, int degree,
                                                rosbag::View::iterator bag_it,
                                                rosbag::View::iterator end,
                                                DeltaCalVariables *input) {
...
//   input->velocity_file = &velocity_file;
  ...
  bag_it = OneSensorCloudsBrass(bag_it, end, &input->k1_buffer,
                                &(input->k1_timestamps), &input->k1_keyframe,
                                &input->k1_timestamp);
  // Key normals
  //   cout << "Calculating Normals" << endl;
  input->k1_key_normal = GetNormals(input->k1_keyframe);
...
rosbag::View::iterator InitializeVariablesSlam(string bag_name, int degree,
                                               rosbag::View::iterator bag_it,
                                               rosbag::View::iterator end,
  ...
  // Read in the first cloud from each kinect, set it as the keyframe
  // and cloud k - 1, set the combined transform to the zero transform
  bag_it = TimeAlignedCloudsSlamBag(
      bag_it, end, &bag_it2, end2, &input->k1_buffer, &input->k2_buffer,
      &(input->k1_timestamps), &input->k2_timestamps, &input->k1_keyframe,
      &input->k2_keyframe, &input->k1_timestamp, &input->k2_timestamp);
  // Key normals
  //   cout << "Calculating Normals" << endl;
  input->k1_key_normal = GetNormals(input->k1_keyframe);
  input->k2_key_normal = GetNormals(input->k2_keyframe);
...
void DeltaCalculationOpenni(const string& bag_name,
  ...
  while (
    ...
    bag_it = TimeAlignedCloudsSlamBag(
        bag_it, end, &bag_it2, end2, &variables->k1_buffer,
        &variables->k2_buffer, &variables->k1_timestamps,
        &variables->k2_timestamps, &k1_cloud, &k2_cloud,
        &variables->k1_timestamp, &variables->k2_timestamp);
    ...
    bool k2_change = CheckChangeVel(variables->k2_combined_transform, degree,
                                    variables->k2_velocity_list);
...
void DeltaCalculation(string bag_name, vector<ros::Publisher> publishers,
  ...
  while (
    ...
    pcl::PointCloud<pcl::PointXYZ> k1_cloud;
    pcl::PointCloud<pcl::PointXYZ> k2_cloud;
    bag_it = TimeAlignedClouds(
        bag_it, end, &variables->k1_buffer, &variables->k2_buffer,
        &variables->k1_timestamps, &variables->k2_timestamps, &k1_cloud,
        &k2_cloud, &variables->k1_timestamp, &variables->k2_timestamp);
    ...
    //     cout << "k1_residual: " << k1_residual << " Time 1: " <<
    //     variables->k1_timestamp << "Time 2: " << variables->k1_prev_timestamp
    //     << endl;
    //     cout << "k2_residual: " << k2_residual << " Time 1: " <<
    //     variables->k2_timestamp << "Time 2: " << variables->k2_prev_timestamp
    //     << endl;
...
void DeltaCalculationSingle(string bag_name, vector<ros::Publisher> publishers,
  ...
  while (((variables->k1_buffer.size() != 0) || (bag_it != variables->end))) {
    ...
    bag_it = OneSensorClouds(bag_it, end, &variables->k1_buffer,
                             &variables->k1_timestamps, &k1_cloud,
...
double *DeltaFromOdom(const vector<double> &current,
  ...
  Eigen::Quaternion<double> previous_quat(previous[4], previous[1], previous[2],
                                          previous[3]);
  ...
  Eigen::Quaternion<double> current_quat(current[4], current[1], current[2],
                                         current[3]);
