...
class NonMarkovLocalization {
 public:
  ...
  // Struct to keep track of the correspondence between observed points and an
  // object instance.
  struct ObjectCorrespondence {
    // Indices of points corresponding to this object instance.
    std::vector<std::size_t> source_points;
    // Indices of the corresponding source poses for each point in
    // @source_points.
    std::vector<std::size_t> source_poses;
    // Index of the object model.
    std::size_t object_model;
    // Index of the object instance.
    std::size_t object_instance;
    // Default constructor.
    ObjectCorrespondence() {}
    // Convenience constructor to set all the values.
    ObjectCorrespondence(
        const std::vector<std::size_t>& _source_points,
        const std::vector<std::size_t>& _source_poses,
        std::size_t _object_model, std::size_t _object_instance) :
        source_points(_source_points), source_poses(_source_poses),
        object_model(_object_model), object_instance(_object_instance) {}
  };
  ...
  struct LocalizationOptions {
    ...
    float kPointObjectCorrelationFactor;
    ...
    bool use_object_constraints;
    unsigned int min_object_overlap;
    ...
    void (*CorrespondenceCallback)(
      ...
      const std::vector<ObjectCorrespondence>&
          point_object_correspondences,
      const std::vector<EnmlMaps::PersistentObject>& objects,
 ...
 private:
  ...
  // Verifies that the previously evaluated object correspondences in
  // object_correspondences_ to observationd from poses [@min_poses, @max_poses]
  // are still valid, and removes those entries from object_correspondences_
  // that are evaluated to be no longer valid object correspondences.
  void VerifyActiveObjects(const std::size_t min_poses,
                           const std::size_t max_poses);

  // Find correspondences between observations from poses min_poses to
  // max_poses, and the object instances.
  void FindObjectCorrespondences(const std::size_t min_poses,
                                 const std::size_t max_poses);

  // Find correspondences between observations from poses
  // [@min_poses, @max_poses] and the object with model @model_id and instance
  // with the specified pose @model_pose. The overlap value of all the
  // observations and the specified model instance is returned in
  // @overlap_value_ptr. The object correspondence passed as @correspondence_ptr
  // is updated to the observations found to correspond to the object instance.
  // If the overlap value is > localization_options_.min_object_overlap, then
  // the observation classes of the corresponding observations is set to
  // kObjectObservation.
  void FindSingleObjectCorrespondence(
      const std::size_t min_poses,
      const std::size_t max_poses,
      const std::size_t model_id,
      const perception_2d::Pose2Df& object_pose,
      ObjectCorrespondence* correspondence_ptr,
      float* overlap_value_ptr);
  ...
  // Add point to object instance constraints based on previously computed
  // correspondences stored in member variable point_object_correspondences_.
  // For each constraint, a new residual will be created and added to the
  // point_object_constraints_ member variable. A new autodiff residual will
  // be created from every constraint and added to the Ceres @problem.
  void AddObjectConstraints(ceres::Problem* problem);
  ...
  // List of Model-Instance objects.
  std::vector<EnmlMaps::PersistentObject> objects_;

  // Array containing the 3DOF poses of object instances detected.
  std::vector<double> objects_pose_array_;

  // List of correspondences between observed points and object instances.
  std::vector<ObjectCorrespondence> object_correspondences_;
  ...
  // List of point to object instance constraints.
  std::vector<PointToObjectConstraint*> point_object_constraints_;
