...
struct HumanImposedConstraint1 {
  ...
  bool operator()(const T* const pose, T* residuals) const {
    ...
    const Eigen::Matrix<T, 2, 1>
      init_trans(T(init_linked_pose.translation.x()) -
                 T(constrained_pose.translation.x()),
                 T(init_linked_pose.translation.y()) -
                 T(constrained_pose.translation.y()));
    T init_rot = T(init_linked_pose.angle) - T(constrained_pose.angle);
    const Eigen::Matrix<T, 2, 1>
      curr_trans(T(curr_linked_pose.translation.x()) - T(pose[0]),
                 T(curr_linked_pose.translation.y()) - T(pose[1]));
    T curr_rot = T(curr_linked_pose.angle) - T(pose[2]);

    //T delta_trans_magnitude = sqrt(()*() + ()*());

 T delta_trans_magnitude = sqrt(T(curr_linked_pose.translation.y()) - pose[1]);

    //T delta_trans_magnitude = pow((curr_trans(0) - init_trans(0)) *
    //                               (curr_trans(0) - init_trans(0)) +
    //                               (curr_trans(1) - init_trans(1)) *
    //                               (curr_trans(1) - init_trans(1)), 0.5);

//    T delta_trans_magnitude = sqrt(T(1.0));
    T delta_rot_magnitude = fabs(curr_rot - init_rot);
    residuals[0] = alpha * delta_trans_magnitude;
    residuals[1] = beta * delta_rot_magnitude;

    residuals[0] *= T(user_confidence);
    residuals[1] *= T(user_confidence);
