...
vector<Vector2f> GrahamsScan::run(vector<Vector2f> points)
{
  ...
  for (int k=1;k<NumPoints;k++){  //FIND MIN POINT
    if(debug)
      printf("%d. %7.4f , %7.4f\n",k,V2COMP(points[k]));
    if ((points[k].y() == points[minPoint].y() &&
        points[k].x() < points[minPoint].x()) ||
...
{
  const Vector2f v1 = P->loc - P->prev->loc;
  const Vector2f v2 = P->next->loc - P->loc;
  const float cross_product = v1.x() * v2.y() - v1.y() * v2.x();
...
void GrahamsScan::addPoint(point& Point)
{
  ...
  while ((tempPoint->angle > curPoint->angle ||
      (tempPoint->angle + Epsilon > curPoint->angle &&
      tempPoint->loc.y() > curPoint->loc.y())) && curPoint->next!=NULL) {
