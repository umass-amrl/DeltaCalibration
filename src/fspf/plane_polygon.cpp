...
PlanePolygon::PlanePolygon(vector<Vector3f> points) :
    normals2D(0), edgeDir2D(0), edgeLengths(0), offsets2D(0), pixelLocs(0),
    vertices(0), vertices2D(0) {
...
PlanePolygon::PlanePolygon(
    vector<Vector3f> points, vector<vector2i> _pixelLocs) :
    normals2D(0), edgeDir2D(0), edgeLengths(0), offsets2D(0), pixelLocs(0),
    vertices(0), vertices2D(0) {
  numPoints = points.size();
  validPolygon = computePlaneParameters(points);
...
bool PlanePolygon::computePlaneParameters(vector<Vector3f>& points)
{
  ...
  normal = Vector3f(
      eigenVectors(0,minInd), eigenVectors(1,minInd), eigenVectors(2,minInd));
  normal.normalize();
  if(normal.dot(p0)<0.0)
    normal = -normal;
  offset = -normal.dot(p0);
  b1 = Vector3f(
      eigenVectors(0,maxInd), eigenVectors(1,maxInd), eigenVectors(2,maxInd));
  b2 = Vector3f(
      eigenVectors(0,midInd), eigenVectors(1,midInd), eigenVectors(2,midInd));
  sum_xx = invN*sum_xx;
  sum_yy = invN*sum_yy;
  sum_zz = invN*sum_zz;
  sum_xy = invN*sum_xy;
...
bool PlanePolygon::constructConvexPoly(vector<Vector3f>& points)
{
  ...
  for(unsigned int i=0; i<points.size(); i++){
    ...
    vertices2D.push_back(p);
  }
  vertices2D = grahamsScan.run(vertices2D);
  ...
  Vector3f p;
  min2D.setConstant(FLT_MAX);
  max2D.setConstant(-FLT_MAX);
  for(unsigned int i=0; i<N; i++){
    ...
    vertices[i] = p;
    normals2D[i] = Perp2<float>(vertices2D[(i+1)%N]-vertices2D[i]).normalized();
...
bool PlanePolygon::liesAlongside(const Vector3f& p) const
{
  ...
  bool interior = true;
  if (normals2D.size() != offsets2D.size() ||
      offsets2D.size() != vertices2D.size()) {
    printf("Vector size mismatch in %s normals2D.size():%d offsets2D.size():%d"
            "vertices2D.size():%d\n",
           __PRETTY_FUNCTION__,int(normals2D.size()), int(offsets2D.size()),
           int(vertices2D.size()));
    exit(1);
  }
...
void PlanePolygon::merge(PlanePolygon& poly2)
{
  ...
  Vector3d p0Double =
      (numPoints * p0 + poly2.numPoints * poly2.p0).cast<double>() /
      newNumPoints;

  // Compute statistics required to construct the MI tensor

  double weight1 = numPoints/newNumPoints;
  double weight2 = poly2.numPoints/newNumPoints;
  ...
  if (normals2D.size() != offsets2D.size() ||
    ...
    printf("normals2D.size():%d offsets2D.size():%d vertices2D.size():%d\n",
           int(normals2D.size()), int(offsets2D.size()),
           int(vertices2D.size()));
    exit(3);
  }
}
...
void PlanePolygon::merge(vector<PlanePolygon>& polygons)
{
  ...
  Vector3d p0Double = numPoints * p0.cast<double>();
  for(unsigned int i=0; i<polygons.size(); i++){
    newNumPoints += polygons[i].numPoints;
    p0Double += polygons[i].numPoints * polygons[i].p0.cast<double>();
  }
  ...
  normal.normalize();
  p0 = p0Double.cast<float>();
  if (normal.dot(p0) <0.0) normal = -normal;
  offset = -normal.dot(p0);
  b1 = Vector3f(
      eigenVectors(0,maxInd), eigenVectors(1,maxInd), eigenVectors(2,maxInd));
  b2 = Vector3f(
  ...
  for(unsigned int i=0; i<vertices.size(); i++){
    const Vector2f p(b1.dot(vertices[i]-p0), b2.dot(vertices[i]-p0));
    points2d.push_back(p);
  }
  for(unsigned int i=0; i<polygons.size(); i++){
    for(unsigned int j=0; j<polygons[i].vertices.size(); j++){
      const Vector2f p(b1.dot(polygons[i].vertices[j]-p0),
                       b2.dot(polygons[i].vertices[j]-p0));
      points2d.push_back(p);
    }
  }
...
void PlanePolygon::transform(Vector3f translation, Quaternionf rotation)
{
  ...
  Rotation2Df R(rotation);
  Translation2f T(translation);
  Affine2f M(T*R);
  transform(M);
}
*/
...
/*
void PlanePolygon::transform(const Affine2f& M)
{
  static const bool debug = false;
  Eigen::Matrix3d R;
  Eigen::Vector3d t(M.m14,M.m24,M.m34);
  Vector3d p = p0.cast<double>();
